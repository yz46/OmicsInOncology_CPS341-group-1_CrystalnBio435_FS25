...
ggplot(dat, aes(x = Time, y = OD600_blank_corrected, color = Experiment, group = Growth_Medium)) +
 #   geom_line(size = 1) +        # Line plot for bacterial growth
  geom_point() +       # Points for clarity
  labs(title = "Bacterial Growth in Different Media",
       x = "Time (hours)",
       y = "OD600 (Blank Corrected)",
       color = "Growth Medium") +
    facet_grid(~ Growth_Medium) +
  theme_minimal()  # Clean theme for better visualization
#  scale_color_manual(values = c("blue", "red", "yellow"))  # Assign colors to media
```
