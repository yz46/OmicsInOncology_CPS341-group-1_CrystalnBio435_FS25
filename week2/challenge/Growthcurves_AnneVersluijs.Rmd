...
# Summarize Data: Mean and Standard Deviation for Each Condition
summary_data <- dat %>%
    group_by(Growth_Medium, Time) %>%
    summarise(
        Mean_OD = mean(OD600_blank_corrected, na.rm = TRUE),
        SD_OD = sd(OD600_blank_corrected, na.rm = TRUE),
        N = n()
    )
...
# Hypothesis Testing: ANOVA to Compare Growth in Different Media
...
# Now its shows all the data points, but we need a regression line
# Load necessary libraries
library(ggplot2)
library(dplyr)

# Ensure Growth_Medium is a factor
